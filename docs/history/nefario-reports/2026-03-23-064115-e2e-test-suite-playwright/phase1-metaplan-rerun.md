# Meta-Plan: E2E Playwright Test Suite for WRL (Revised)
...
- **Why this agent**: test-minion owns test strategy, test architecture, and CI test pipeline design. The core planning challenge is how to structure e2e tests against a remote Workers environment with OAuth, queues, and shared state. Also best positioned to advise on what documentation the test suite itself needs (README, env setup, run instructions) as implementation artifacts.
...
- **Why this agent**: ux-strategy-minion ensures the tests map to real user jobs-to-be-done, not just API endpoint coverage. Catches gaps where the test suite misses critical user paths, and helps prioritize tests by journey criticality.

## Cross-Cutting Checklist

- **Testing**: INCLUDED -- test-minion is the primary planning consultant (Consultation 1). This is fundamentally a testing task.
- **Security**: INCLUDED -- security-minion consulted for test credential management, staging auth model, and webhook receiver security posture (Consultation 3). Tests require secrets in CI and interact with OAuth flows.
- **Usability -- Strategy**: INCLUDED -- ux-strategy-minion reviews journey coherence, identifies gaps in the test scenarios, and provides priority ranking (Consultation 5).
...
- **Documentation**: NOT INCLUDED as a separate planning consultation. Rationale: the test suite's documentation needs (README, env setup, run instructions) are implementation artifacts best defined by the agents producing the tests and CI workflow. test-minion's planning question (item f) explicitly addresses what docs should accompany the tests, and iac-minion's CI workflow design implicitly covers secret setup documentation. No standalone docs architecture question remains that warrants a separate planning agent. Documentation coverage will be verified during execution planning and post-execution Phase 8.
- **Observability**: NOT INCLUDED for planning -- the test suite itself is not a production runtime component. CI test results and artifacts are standard Playwright/GitHub Actions concerns handled by iac-minion and test-minion.
...
The five specialists were chosen because this task sits at the intersection of:
- **Test engineering** (test-minion): core domain -- Playwright configuration, test isolation, auth patterns against remote staging, and test-adjacent documentation (README, setup instructions)
- **CI infrastructure** (iac-minion): GitHub Actions workflow, browser caching, secrets management, artifact upload
- **Security** (security-minion): credentials in CI, staging auth model, test GitHub account implications, webhook receiver security
- **API design** (api-design-minion): webhook receiver architecture for delivery verification testing
- **UX strategy** (ux-strategy-minion): ensuring tests map to real user journeys, not just endpoint coverage; prioritizing tests by journey criticality

The removal of software-docs-minion from the planning team is justified because the documentation deliverables for this task are implementation artifacts (test README, env var docs, CI setup notes) rather than architectural or API documentation. These are best specified by the agents who define the test architecture and CI workflow. Post-execution Phase 8 will still assess documentation completeness.
