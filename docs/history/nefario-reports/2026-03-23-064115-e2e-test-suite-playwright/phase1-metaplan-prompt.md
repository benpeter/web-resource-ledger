...
## Codebase Context
- Cloudflare Workers app (src/index.js) using D1, R2, KV, Queues, Browser bindings
- Existing unit tests in `test/` (vitest, ~40 test files) + integration tests in `test/integration/`
- CI: `.github/workflows/ci.yml` runs unit + integration tests
- Staging env: wrl-staging.benpeter.workers.dev (separate D1, R2, KV, queues)
- OAuth flow: GitHub OAuth with session cookies (src/oauth.js)
- API routes: /v1/captures, /v1/captures/batch, /v1/webhooks, /v1/verify, /auth/* etc.
- Webhook system: queue-based with DLQ retry (src/webhook-dispatch.js)
- Quota system: per-tenant quotas with 429 responses (src/quotas.js)
- Verify page: public HTML page at /v1/verify/:id (src/verify-page.js)
- No existing e2e tests or Playwright config
- Package uses @cloudflare/playwright for browser captures (not for testing)

## External Skill Discovery
No external skills discovered (.claude/skills/ and .skills/ directories do not exist).
...
7. Write your complete meta-plan to `/var/folders/3k/bfjvvz9s6dvdn_hvlhvr8lc00000gn/T//nefario-scratch-wXsjF2/e2e-test-suite-playwright/phase1-metaplan.md`
